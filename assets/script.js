var owmAPI = "1840f1268ab13ea50f20dc46d7756874";
var currentCity = "";
var lastCity = "";
...
var getCurrentConditions = (event) => {
    let city = $('#searchit').val();
    currentCity= $('#searchit').val();
...
var renderCities = () => {
    ...
    if (localStorage.length===0){
        if (lastCity){
            $('#searchit').attr("value", lastCity);
        } else {
            $('#searchit').attr("value", "Austin");
        }
    } else {
...
$('#city-results').on("click", (event) => {
    event.preventDefault();
    $('#searchit').val(event.target.textContent);
    currentCity=$('#searchit').val();
    getCurrentConditions(event);
});
